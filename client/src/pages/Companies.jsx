...
export default function Companies() {
  ...
  return (
    <main className="container">
      <h1 style={{ fontSize: 30, marginBottom: 6 }}>{t('companies.title')}</h1>
      <p className="muted" style={{ marginBottom: 20 }}>{t('companies.subtitle')}</p>

      {companies.length === 0 ? (
        <Card><p className="muted">{t('companies.empty')}</p></Card>
      ) : (
              ...
              <Card>
                <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
                  ...
                  <div>
                    ...
                    <p className="muted" style={{ marginTop: 2 }}>{t(c.open_jobs === 1 ? 'companies.openPositionOne' : 'companies.openPositionOther', { n: c.open_jobs })}</p>
                  </div>
                </div>
                {c.description && <p className="muted" style={{ marginTop: 12 }}>{snippets[i]}</p>}
                {c.open_jobs > 0 && <Badge variant="verified" style={{ marginTop: 12 }}>{t('companies.hiringNow')}</Badge>}
              </Card>
            </Link>
          ))}
        </div>
